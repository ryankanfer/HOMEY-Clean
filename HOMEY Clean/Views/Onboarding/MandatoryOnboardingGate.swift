...
struct MandatoryOnboardingView: View {
    ...
    var body: some View {
        ZStack {
            ...
            // Escape attempt warning
            if attemptedToEscape {
                EscapeAttemptWarning {
                    attemptedToEscape = false
                }
            }
        }
        .onTapGesture {
            // Detect tap outside onboarding content
            handleEscapeAttempt()
        }
        .gesture(
        ...
        .onReceive(NotificationCenter.default.publisher(for: UIApplication.willResignActiveNotification)) { _ in
            // Prevent app backgrounding during onboarding
            handleEscapeAttempt()
        }
    }
    
    private func handleEscapeAttempt() {
        withAnimation(.spring()) {
            attemptedToEscape = true
        }
