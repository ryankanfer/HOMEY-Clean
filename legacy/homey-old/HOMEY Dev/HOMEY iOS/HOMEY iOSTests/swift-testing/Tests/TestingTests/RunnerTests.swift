...
func parameterizedWithAsyncThrowingArgs(i _: Int) {}
...
func parameterizedWithFatalArguments(i _: Int) {}
    ...
    final class RunnerTests: XCTestCase {
        ...
        func testYieldsIssueWhenErrorThrownFromTestCase() async throws {
            ...
            var configuration = Configuration()
            configuration.eventHandler = { event, _ in
                if case let .issueRecorded(issue) = event.kind, let error = issue.error as? MyParameterizedError,
                   error.index == randomNumber
                {
                    errorObserved.fulfill()
                }
            }
        ...
        func testParameterizedTestWithNoCasesIsSkipped() async throws {
            ...
            let runner = await Runner(testing: [
                Test(arguments: [Int](), parameters: [.init(index: 0, firstName: "i", type: Int.self)]) { _ in },
            ], configuration: configuration)
            await runner.run()
            await fulfillment(of: [testSkipped], timeout: 0.0)
        }
        ...
        func testTestActionIsRecordIssueDueToErrorThrownByConditionTrait() async throws {
            ...
            let runner = try await Runner(testing: [
                Test(.enabled(if: sketchyCondition(), "Some comment")) {},
            ], configuration: configuration)
            await runner.run()
        ...
        func testErrorThrownWhileEvaluatingArguments() async throws {
            ...
            configuration.eventHandler = { event, _ in
                ...
                if case let .issueRecorded(issue) = event.kind, let error = issue.error as? MyDescriptiveError,
                   String(describing: error) == "boom"
                {
                    errorObserved.fulfill()
                }
            }
        ...
        func testUnavailableTestsAreSkipped() async throws {
            ...
            func testObsoletedTestFunctions() async throws {
                ...
                for step in plan.steps where !step.test.isSuite {
                    XCTAssertNotNil(step.test.comments(from: ConditionTrait.self).map(\.rawValue)
                        .first { $0.contains("999.0") }
                    )
                }
            }
        #endif
        ...
        func testAvailableWithDefinedAvailability() async throws {
            ...
            @Suite(.hidden) struct MainActorIsolationTests {
                ...
                @Test(.hidden, arguments: 0 ..< 10) func mightRunOnMainActor(arg _: Int) {
                    XCTAssertEqual(Thread.isMainThread, isMainActorIsolationEnforced)
                }
