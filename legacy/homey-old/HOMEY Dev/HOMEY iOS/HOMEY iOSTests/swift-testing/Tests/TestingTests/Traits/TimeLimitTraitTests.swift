...
struct TimeLimitTraitTests {
    ...
    func timesOutDueToTrait() async throws {
    ...
    func timesOutDueToInheritedTrait() async throws {
        await confirmation("Issue recorded", expectedCount: 10) { issueRecorded in
            var configuration = Configuration()
            configuration.testTimeLimitGranularity = .milliseconds(1)
    ...
    func timesOutDueToDefaultTimeLimit() async throws {
        await confirmation("Issue recorded", expectedCount: 10) { issueRecorded in
            var configuration = Configuration()
            configuration.testTimeLimitGranularity = .milliseconds(1)
    ...
    func timesOutDueToMaximumTimeLimit() async throws {
        await confirmation("Issue recorded", expectedCount: 10) { issueRecorded in
            var configuration = Configuration()
            configuration.testTimeLimitGranularity = .milliseconds(1)
            configuration.maximumTestTimeLimit = .milliseconds(10)
    ...
    @Test(
        "Issue.Kind.timeLimitExceeded.description property",
        arguments: [
            (123, 0, "123.000"),
