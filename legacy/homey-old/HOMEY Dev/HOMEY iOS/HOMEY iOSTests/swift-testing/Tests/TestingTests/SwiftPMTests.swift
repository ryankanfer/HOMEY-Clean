...
struct SwiftPMTests {
    ...
    @Test(
        "--symbolicate-backtraces argument",
        arguments: [
    ...
    func filterAndSkipAndHidden() async throws {
        ...
        func xunitOutputWithBadPath() {
            ...
            #expect(throws: CError.self) {
                _ = try configurationForEntryPoint(withArguments: [
                    "PATH",
                    "--xunit-output",
                    "/nonexistent/path/we/cannot/write/to",
                ])
            }
        }
    ...
    func list() async throws {
        ...
        let currentTestID = try String(describing: #require(Test.current?.id.parent))
    ...
    func listVerbose() async throws {
        ...
        let currentTestID = try String(describing: #require(Test.current?.id))
        #expect(testIDs.contains(currentTestID))
