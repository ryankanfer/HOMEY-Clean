...
struct ParallelizationTraitTests {
    ...
    func serializedTrait() async {
        ...
        for step in plan.steps {
            #expect(
                step.action.isParallelizationEnabled == false,
                "Step \(step) should have had parallelization disabled"
            )
        }
    }
    ...
    func serializesParameterizedTestFunction() async {
        ...
        #expect(indicesRecordedValue.count == 10000)
...
private struct OuterSuite {
    /* This @Suite intentionally left blank */ enum IntermediateSuite {
        @Suite(.hidden)
        struct InnerSuite {
            @Test(.hidden) func example() {}

            @Test(.hidden, arguments: 0 ..< 10000) func parameterized(i: Int) async throws {
