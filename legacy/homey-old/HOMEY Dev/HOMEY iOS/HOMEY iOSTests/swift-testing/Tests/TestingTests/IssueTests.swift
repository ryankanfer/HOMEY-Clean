...
private import _TestingInternals
    ...
    final class IssueTests: XCTestCase {
        ...
        enum TypeWithMemberFunctions {
            static func f(_: Int) -> Bool { false }
            static func g(label _: Int) -> Bool { false }
            static func h(_: () -> Void) -> Bool { false }
            static func j(_: Int) -> Never? { nil }
            static func k(_: inout Int) -> Bool { false }
            static func m(_: Bool) -> Bool { false }
            static func n(_: Int) throws -> Bool { false }
        }
        ...
        func testThrowingMemberFunctionCall() async throws {
            ...
            await Test { () throws in
                ...
                #expect(try TypeWithMemberFunctions.f({ () throws in 0 }()))
            }.run(configuration: configuration)
        ...
        func testExpressionLiterals() async {
            func expectIssue(
                ...
                in testFunction: @escaping @Sendable () async throws -> Void
            ) async {
        ...
        func testExpressionRuntimeValueCapture() throws {
            ...
            do {
                ...
                XCTAssertEqual(
                    runtimeValue.typeInfo.fullyQualifiedName,
                    "TestingTests.IssueTests.ExpressionRuntimeValueCapture_Value"
                )
                XCTAssertFalse(runtimeValue.isCollection)
            }
        ...
        func testExpressionRuntimeValueChildren() throws {
            ...
            do {
                expression = expression
                    .capturingRuntimeValues(ExpressionRuntimeValueCapture_ValueWithChildren(contents: [
                        123,
                        "abc",
                    ]))
                let runtimeValue = try XCTUnwrap(expression.runtimeValue)
                XCTAssertEqual(
                    String(describing: runtimeValue),
                    #"ExpressionRuntimeValueCapture_ValueWithChildren(contents: [123, "abc"])"#
                )
                XCTAssertEqual(
                    runtimeValue.typeInfo.fullyQualifiedName,
                    "TestingTests.IssueTests.ExpressionRuntimeValueCapture_ValueWithChildren"
                )
                XCTAssertFalse(runtimeValue.isCollection)
                XCTAssertNil(runtimeValue.label)
        ...
        func testErrorCheckingWithExpect_mismatchedErrorDescription_nonVoid() async throws {
            ...
            configuration.eventHandler = { event, _ in
                ...
                XCTAssertEqual(
                    expectation.mismatchedErrorDescription,
                    "an error was expected but none was thrown and \"0\" was returned"
                )
                expectationFailed.fulfill()
            }
        ...
        func testDescriptionProperties() {
            do {
                let sourceLocation = SourceLocation(
                    fileID: "FakeModule/FakeFile.swift",
                    filePath: "",
                    line: 9999,
                    column: 1
                )
                let issue = Issue(
                    kind: .system,
                    comments: ["Some issue"],
                    sourceContext: SourceContext(sourceLocation: sourceLocation)
                )
            ...
            do {
                let issue = Issue(
                    kind: .system,
                    comments: ["Some issue"],
                    sourceContext: SourceContext(sourceLocation: nil)
                )
        ...
        func testCollectionDifferenceSkippedForRanges() async {
            ...
            await Test {
                ...
                #expect(range_uint16 == -1 ... 65535, "incorrect min")
                #expect(range_uint16 == 0 ... 65534, "incorrect max")
        ...
        func testNegatedExpressionsExpandToCaptureNegatedExpression() async {
            ...
            configuration.eventHandler = { event, _ in
                ...
                guard case let .negation(subexpression, isParenthetical) = expectation.evaluatedExpression.kind else {
                    XCTFail(
                        "Expected expression's kind was negation, but it was \(expectation.evaluatedExpression.kind)"
                    )
                    return
                }
    ...
    struct IssueCodingTests {
        private static let issueKinds: [Issue.Kind] = [
            ...
            Issue.Kind.expectationFailed(Expectation(
                evaluatedExpression: .__fromSyntaxNode("abc"),
                isPassing: true,
                isRequired: true,
        ...
        func codable(issueKind: Issue.Kind) async throws {
            let issue = Issue(
                kind: issueKind,
                comments: ["Comment"],
        ...
        @Test(
            "Custom descriptions are the same",
            arguments: issueKinds
        )
...
private extension Optional {
    static func memberAccessWithInferredBase(_ this: Self) -> Self {
        this
    }
}
