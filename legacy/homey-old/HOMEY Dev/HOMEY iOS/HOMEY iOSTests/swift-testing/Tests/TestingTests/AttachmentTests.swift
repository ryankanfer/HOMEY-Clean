...
struct AttachmentTests {
    @Test func saveValue() {
    ...
    #if !SWT_NO_FILE_IO
        func compare(
            _ attachableValue: borrowing MySendableAttachable,
            toContentsOfFileAtPath filePath: String
        ) throws {
        ...
        @Test func writeAttachmentWithNameConflict() throws {
            ...
            for i in 0 ..< 5 {
                ...
                // Write the attachment to disk, then read it back.
                let filePath = try attachment.write(
                    toFileInDirectoryAtPath: temporaryDirectory(),
                    appending: suffixes.next()!
                )
    ...
    @Test func issueRecordedWhenAttachingNonSendableValueThatThrows() async {
        ...
        @Test(
            "Attach Codable- and NSSecureCoding-conformant values",
            .serialized,
            arguments: CodableAttachmentArguments.all()
        )
...
struct MyAttachable: Attachable, ~Copyable {
    ...
    func withUnsafeBufferPointer<R>(
        for _: borrowing Attachment<Self>,
