...
struct EventRecorderTests {
    ...
    func quietOutput() async throws {
        ...
        func issueCountSummingAtTestEnd(
            testName: String,
            isSuite: Bool,
            issueCount: (total: Int, expected: Int)
        ) async throws {
            let stream = Stream()
    ...
    func issueCountOmittedForPassingTest() async throws {
        ...
        await Test(name: "Innocuous Test Name") {}.run(configuration: configuration)
        ...
        func junitXMLWithTimelessSkippedTest() async throws {
            ...
            eventRecorder.record(
                ...
                in: Event.Context(test: nil, testCase: nil, configuration: nil)
            )
            let test = Test {}
            eventRecorder.record(
                Event(.testSkipped(.init(sourceContext: .init())), testID: test.id, testCaseID: nil),
                in: Event.Context(test: test, testCase: nil, configuration: nil)
            )
            eventRecorder.record(
...
@Suite("Animal Crackers", .hidden) struct WrittenTests {
    ...
    @Test(.hidden, .tags(.red, .orange, .green), arguments: 0 ..< 10) func severalLarks(i _: Int) {}
