...
struct PlanTests {
    ...
    func selectedTestsByAnyTag() async throws {
        ...
        let innerTestType = try #require(await test(for: SendableTests.NestedSendableTests.self))
        let testB = try #require(await testFunction(named: "succeeds()", in: SendableTests.NestedSendableTests.self))
        let testC = try #require(await testFunction(
            named: "otherSucceeds()",
            in: SendableTests.NestedSendableTests.self
        ))
    ...
    func recursiveTraitApplication() async throws {
        ...
        let deeplyNestedTest = try #require(await testFunction(
            named: "example()",
            in: OuterTest.IntermediateType.InnerTest.self
        ))
    ...
    func staticFunctionIsNestedAtSameLevelAsInstanceFunction() async throws {
        ...
        #expect(plan.stepGraph
            .subgraph(at: typeInfo.fullyQualifiedNameComponents + CollectionOfOne("succeeds()")) != nil
        )
        #expect(plan.stepGraph.subgraph(at: typeInfo.fullyQualifiedNameComponents + CollectionOfOne("static()")) != nil)
        #expect(plan.stepGraph
            .subgraph(at: typeInfo.fullyQualifiedNameComponents + CollectionOfOne("reserved1(reserved2:)")) != nil
        )
    }
    ...
    struct ContainingSuiteSynthesis {
        ...
        func oneImplicitParent() async throws {
            ...
            let implicitParentSuite_A = try #require(plan.steps.map(\.test)
                .first { $0.name == "ImplicitParentSuite_A" }
            )
        ...
        @Test(
            "A test function in a type hierarchy where the nearest suite is explicit and outer ones are implicit",
            arguments: [
        ...
        func twoImplicitAncestorsButExplicitParent(suiteType: Any.Type) async throws {
            ...
            let implicitParentSuite_B = try #require(plan.steps.map(\.test)
                .first { $0.name == "ImplicitParentSuite_B" }
            )
            ...
            let implicitGrandparentSuite_B = try #require(plan.steps.map(\.test)
                .first { $0.name == "ImplicitGrandparentSuite_B" }
            )
        ...
        func mixedAncestors() async throws {
            ...
            let implicitParentSuite_C = try #require(plan.steps.map(\.test)
                .first { $0.name == "ImplicitParentSuite_C" }
            )
            ...
            let explicitGrandparentSuite_C = try #require(plan.steps.map(\.test)
                .first { $0.name == "ExplicitGrandparentSuite_C" }
            )
        ...
        func allImplicitAncestors() async throws {
            ...
            let testFunction = try #require(plan.steps.map(\.test).first { $0.name == "example()" })
            let implicitGrandchildSuite_D = try #require(plan.steps.map(\.test)
                .first { $0.name == "ImplicitGrandchildSuite_D" }
            )
            ...
            let implicitParentSuite_D = try #require(plan.steps.map(\.test)
                .first { $0.name == "ImplicitParentSuite_D" }
            )
            #expect(implicitParentSuite_D.sourceLocation == implicitChildSuite_D.sourceLocation)

            let implicitGrandparentSuite_D = try #require(plan.steps.map(\.test)
                .first { $0.name == "ImplicitGrandparentSuite_D" }
            )
...
private struct ImplicitParentSuite_A {
    @Test(.hidden) func example() {}
}

private enum ImplicitGrandparentSuite_B {
    // This fixture must not have an explicit `@Suite` attribute to validate suite
    // synthesis. Its children can be `.hidden`, though.
    enum ImplicitParentSuite_B {
...
private struct ExplicitGrandparentSuite_C {
    // This fixture must not have an explicit `@Suite` attribute to validate suite
    // synthesis. Its children can be `.hidden`, though.
    enum ImplicitParentSuite_C {
...
// suite synthesis.
private enum ImplicitGrandparentSuite_D {
    enum ImplicitParentSuite_D {
        enum ImplicitChildSuite_D {
