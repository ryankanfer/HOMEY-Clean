    ...
    struct ABIEntryPointTests {
        ...
        private func withTestingLibraryImageAddress<R>(_ body: (ImageAddress?) throws -> R) throws -> R {
            ...
            var testingLibraryAddress: ImageAddress?
                ...
                var info = Dl_info()
                try #require(dladdr(addressInTestingLibrary, &info) != 0)
            ...
            // When using glibc, dladdr() is only available if __USE_GNU is specified.
                ...
                try addressInTestingLibrary.withMemoryRebound(
                    to: wchar_t.self,
                    capacity: MemoryLayout<UnsafeRawPointer>.stride / MemoryLayout<wchar_t>.stride
                ) { addressInTestingLibrary in
            ...
            #else
                #warning(
                    "Platform-specific implementation missing: cannot find the testing library image the test suite is linked against"
                )
            #endif
