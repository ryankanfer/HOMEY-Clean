...
struct DiagnosticMessage: SwiftDiagnostics.DiagnosticMessage {
    ...
    ///
    /// - Returns: A diagnostic message.
    static func genericDeclarationNotSupported(
        _ decl: some SyntaxProtocol,
        whenUsing attribute: AttributeSyntax,
        becauseOf genericClause: some SyntaxProtocol,
        on genericDecl: some SyntaxProtocol
    ) -> Self {
        if Syntax(decl) != Syntax(genericDecl), genericDecl.isProtocol((any DeclGroupSyntax).self) {
            return .containingNodeUnsupported(
                genericDecl,
                genericBecauseOf: Syntax(genericClause),
                whenUsing: attribute,
                on: decl
            )
        } else {
    ...
    static func availabilityAttributeNotSupported(
        _ availabilityAttribute: AttributeSyntax,
    ...
    ///
    /// - Returns: A diagnostic message.
    static func nonMemberTagDeclarationNotSupported(
    ...
    ///
    /// - Returns: A diagnostic message.
    static func mistypedTagDeclarationNotSupported(
        _ attribute: AttributeSyntax,
        on decl: VariableDeclSyntax,
        declaredType: TypeSyntax,
        resolvedType: TypeSyntax? = nil
    ) -> Self {
    ...
    ///
    /// - Returns: A diagnostic message.
    static func containingNodeUnsupported(
        _ node: some SyntaxProtocol,
        genericBecauseOf genericClause: Syntax? = nil,
        whenUsing attribute: AttributeSyntax,
        on decl: some SyntaxProtocol,
        withSuppressedConformanceToEscapable escapableNonConformance: SuppressedTypeSyntax? = nil
    ) -> Self {
        // Avoid using a syntax node from a lexical context (it won't have source
        // location information.)
    ...
    ///
    /// - Returns: A diagnostic message.
    static func attributeNotSupportedOutsideTagExtension(
        _ attribute: AttributeSyntax,
        on decl: VariableDeclSyntax
    ) -> Self {
        Self(
    ...
    ///
    /// - Returns: A diagnostic message.
    static func attributeArgumentCountIncorrect(
        _ attribute: AttributeSyntax,
        on functionDecl: FunctionDeclSyntax
    ) -> Self {
    ...
    private static func _addArgumentsFixIts(
        for attribute: AttributeSyntax,
        given parameters: FunctionParameterListSyntax
    ) -> [FixIt] {
        ...
        do {
            ...
            addFixIt(
                ...
                appendingArguments: [LabeledExprSyntax(
                    label: "arguments",
            ...
            let additionalArguments = parameters.indices.map { index in
                ...
                return LabeledExprSyntax(
                    ...
                    trailingComma: parameters.index(after: index) < parameters
                        .endIndex ? .commaToken(trailingTrivia: .space) : nil
                )
            }
            addFixIt(
                "Add 'arguments:' with all combinations of \(parameters.count) collections",
                appendingArguments: additionalArguments
            )
        }
    ...
    static func specifierNotSupported(
        ...
        on parameter: FunctionParameterSyntax,
        whenUsing attribute: AttributeSyntax
    ) -> Self {
        Self(
    ...
    ///
    /// - Returns: A diagnostic message.
    static func returnTypeNotSupported(
        _ returnType: TypeSyntax,
        on decl: some SyntaxProtocol,
        whenUsing _: AttributeSyntax
    ) -> Self {
        Self(
    ...
    ///
    /// - Returns: A diagnostic message.
    static func urlExprNotValid(
        _ urlExpr: StringLiteralExprSyntax,
        in traitExpr: FunctionCallExprSyntax,
        ...
        return Self(
            ...
            fixIts: [
                FixIt(
                    ...
                    changes: [.replace(
                        oldNode: Syntax(urlExpr),
                        newNode: Syntax(EditorPlaceholderExprSyntax("url", type: "String"))
                    )]
                ),
                FixIt(
    ...
    ///
    /// - Returns: A diagnostic message.
    static func traitHasNoEffect(_ traitExpr: some ExprSyntaxProtocol, in _: AttributeSyntax) -> Self {
        Self(
        ...
        Self(
            ...
            fixIts: [
                FixIt(
                    ...
                    changes: [.replace(
                        oldNode: Syntax(boolExpr),
                        newNode: Syntax("\(boolExpr) as Bool?" as ExprSyntax)
                    )]
                ),
                FixIt(
                    ...
                    changes: [.replace(
                        oldNode: Syntax(boolExpr),
                        newNode: Syntax("\(boolExpr) ?? false" as ExprSyntax)
                    )]
                ),
            ]
        )
    ...
    ///
    /// - Returns: A diagnostic message.
    static func nonOptionalRequireIsRedundant(
        _ expr: ExprSyntax,
        in macro: some FreestandingMacroExpansionSyntax
    ) -> Self {
    ...
    ///
    /// - Returns: A diagnostic message.
    static func requireThrowsNeverIsRedundant(
        _ expr: ExprSyntax,
        in macro: some FreestandingMacroExpansionSyntax
    ) -> Self {
        // We do not provide fix-its because we cannot see the leading "try" keyword
    ...
    ///
    /// - Returns: A diagnostic message.
    static func checkUnsupported(
        _ checkMacro: some FreestandingMacroExpansionSyntax,
        inExitTest exitTestMacro: some FreestandingMacroExpansionSyntax
    ) -> Self {
        Self(
