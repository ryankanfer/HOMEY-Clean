...
private func _diagnoseTrivialBooleanValue(
...
private func _parseCondition(
    from _: ExprSyntax,
    leftOperand lhs: ExprSyntax,
    operator op: BinaryOperatorExprSyntax,
    rightOperand rhs: ExprSyntax,
    for _: some FreestandingMacroExpansionSyntax,
...
/// - Returns: An instance of ``Condition`` describing `expr`.
private func _parseCondition(
    from expr: FunctionCallExprSyntax,
    for _: some FreestandingMacroExpansionSyntax,
    in _: some MacroExpansionContext
) -> Condition {
    ...
    if inOutArguments.count > 1 {
        ...
        return Condition(expression: expr)
    } else if !inOutArguments.isEmpty && inOutArguments.count != argumentList.count {
...
/// - Returns: An instance of ``Condition`` describing `expr`.
private func _parseCondition(
    from expr: MemberAccessExprSyntax,
    for _: some FreestandingMacroExpansionSyntax,
    in _: some MacroExpansionContext
) -> Condition {
...
/// - Returns: An instance of ``Condition`` describing `expr`.
private func _parseCondition(
    negating expr: ExprSyntax,
    isParenthetical: Bool,
    for macro: some FreestandingMacroExpansionSyntax,
    in context: some MacroExpansionContext
) -> Condition {
...
private func _parseCondition(
       ...
       let op = infixOperator.operator.as(BinaryOperatorExprSyntax.self)
    {
        return _parseCondition(
            from: expr,
            leftOperand: infixOperator.leftOperand,
            operator: op,
            rightOperand: infixOperator.rightOperand,
            for: macro,
            in: context
        )
    }
    ...
    if let negatedExpr = _negatedExpression(expr) {
        return _parseCondition(
            negating: negatedExpr.0,
            isParenthetical: negatedExpr.isParenthetical,
            for: macro,
            in: context
        )
    }
