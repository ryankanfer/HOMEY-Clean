...
private func _createAvailabilityTraitExpr(
    ...
    case (.introduced, false):
        return """
        .__available(\(literal: availability.platformName!
            .textWithoutBackticks
        ), introduced: \(version), message: \(message), sourceLocation: \(sourceLocationExpr)) {
    ...
    case (.obsoleted, false):
        ...
        .__available(\(literal: availability.platformName!
            .textWithoutBackticks
        ), obsoleted: \(version), message: \(message), sourceLocation: \(sourceLocationExpr)) {
    ...
    case (.unavailable, false):
        if let platformName = availability.platformName {
            return """
            .__available(\(literal: platformName
                .textWithoutBackticks
            ), obsoleted: nil, message: \(message), sourceLocation: \(sourceLocationExpr)) {
    ...
    default:
        fatalError(
            "Unsupported keyword \(whenKeyword) passed to \(#function). Please file a bug report at https://github.com/swiftlang/swift-testing/issues/new"
        )
    }
}
