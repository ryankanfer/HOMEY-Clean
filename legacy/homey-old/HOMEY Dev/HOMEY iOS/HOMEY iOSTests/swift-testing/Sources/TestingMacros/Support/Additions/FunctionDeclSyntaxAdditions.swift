...
extension FunctionDeclSyntax {
    ...
    var xcTestCompatibleSelector: ObjCSelectorPieceListSyntax? {
        ...
        if let objcAttribute, case let .objCName(objCName) = objcAttribute.arguments {
            if objCName.first?.name?.textWithoutBackticks.hasPrefix("test") == true {
                return objCName
            }
            return nil
        }
...
extension FunctionParameterSyntax {
    ...
    fileprivate var testFunctionParameter: TupleExprSyntax {
        TupleExprSyntax {
            LabeledExprSyntax(
                label: "firstName",
                expression: StringLiteralExprSyntax(content: firstName.textWithoutBackticks)
            )

            if let secondName {
                LabeledExprSyntax(
                    label: "secondName",
                    expression: StringLiteralExprSyntax(content: secondName.textWithoutBackticks)
                )
            } else {
    ...
    private var typeMetatypeExpression: some ExprSyntaxProtocol {
        ...
        let metatypeMemberAccessBase =
            TupleExprSyntax(elements: [LabeledExprSyntax(expression: baseTypeDeclReferenceExpr)])
