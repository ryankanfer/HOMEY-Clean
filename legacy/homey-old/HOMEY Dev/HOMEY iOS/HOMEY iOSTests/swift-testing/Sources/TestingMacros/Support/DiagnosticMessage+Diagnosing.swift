...
///   - context: The macro context in which the expression is being parsed.
private func _diagnoseIssuesWithTagsTrait(
    _ traitExpr: FunctionCallExprSyntax,
    addedTo attributeInfo: AttributeInfo,
...
///   - context: The macro context in which the expression is being parsed.
private func _diagnoseIssuesWithBugTrait(
    _ traitExpr: FunctionCallExprSyntax,
    addedTo attributeInfo: AttributeInfo,
    in context: some MacroExpansionContext
) {
    ...
    guard let urlArg = traitExpr.arguments.first, urlArg.label == nil,
          ...
          let urlString = stringLiteralExpr.representedLiteralValue
    else {
        return
    }
...
///   - attributeInfo: The `@Test` or `@Suite` attribute.
///   - context: The macro context in which the expression is being parsed.
private func _diagnoseIssuesWithParallelizationTrait(
    _ traitExpr: MemberAccessExprSyntax,
    addedTo attributeInfo: AttributeInfo,
    in context: some MacroExpansionContext
) {
...
func diagnoseIssuesWithLexicalContext(
    ...
    } else if [.arrayType, .dictionaryType, .optionalType, .implicitlyUnwrappedOptionalType]
        .contains(lexicalContext.type.kind)
    {
    ...
    if let attributedDecl = lexicalContext.asProtocol((any WithAttributesSyntax).self) {
        ...
        if !availabilityAttributes.isEmpty {
            ...
            for availabilityAttribute in availabilityAttributes {
                diagnostics.append(.availabilityAttributeNotSupported(
