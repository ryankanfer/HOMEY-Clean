...
public struct TestDeclarationMacro: PeerMacro, Sendable {
    ...
    private static func _diagnoseIssues(
        ...
        diagnostics += diagnoseIssuesWithLexicalContext(
            lexicalContext,
        ...
        {
            diagnostics.append(.containingNodeUnsupported(
                containingTypeDecl,
                whenUsing: testAttribute,
                on: declaration
            ))
        }
            ...
            if let parameterType = parameter.type.as(AttributedTypeSyntax.self) {
                for specifier in parameterType.specifiers {
                    ...
                    if invalidSpecifierKeywords.contains(specifier.specifier.tokenKind) {
                        diagnostics.append(.specifierNotSupported(
                            specifier.specifier,
                            on: parameter,
                            whenUsing: testAttribute
                        ))
        ...
        if let genericClause = function.genericParameterClause {
            diagnostics.append(.genericDeclarationNotSupported(
                function,
                whenUsing: testAttribute,
                becauseOf: genericClause,
                on: function
            ))
           ...
           let inheritedTypes = containingType.inheritanceClause?.inheritedTypes
        {
            ...
            for escapableNonConformance in escapableNonConformances {
                diagnostics.append(.containingNodeUnsupported(
                    ...
                    whenUsing: testAttribute,
                    on: function,
                    withSuppressedConformanceToEscapable: escapableNonConformance
                ))
            }
        }
    ...
    private static func _createThunkDecl(
        ...
        lazy var isMainActorIsolated = !functionDecl.attributes(named: "MainActor", inModuleNamed: "_Concurrency")
            .isEmpty
        ...
        if functionDecl.availability(when: .unavailable).first(where: { $0.platformVersion == nil }) != nil {
            ...
            if functionDecl.isStaticOrClass {
                ...
                if let selectorExpr {
                    ...
                    thunkBody = """
                    if try await Testing.__invokeXCTestCaseMethod(\(selectorExpr), onInstanceOf: \(typeName
                    ).self, sourceLocation: \(sourceLocationExpr)) {
                      return
                    }
                    \(thunkBody)
                    """
        ...
        if functionDecl.signature.effectSpecifiers?.asyncSpecifier == nil && !isMainActorIsolated && !functionDecl
            .isNonisolated
        {
            ...
            let thunkParamsExprCopy = FunctionParameterClauseSyntax {
                ...
                FunctionParameterSyntax(
                    ...
                    defaultValue: InitializerClauseSyntax(
                        value: "Testing.__defaultSynchronousIsolationContext" as ExprSyntax
                    )
                )
            }
    ...
    private static func _createTestContainerDecls(
        ...
        if attributeInfo.hasFunctionArguments && !functionDecl.availabilityAttributes.isEmpty {
            ...
            result.append(
                ...
                private \(_staticKeyword(for: typeName
                )) nonisolated func \(unavailableTestName)() async -> [Testing.Test] {
                  [
                    .__function(
                      named: \(literal: functionDecl.completeName),
                      in: \(typeNameExpr),
