...
enum Environment {
    ...
    private static func _splitEnvironmentVariable(_ row: String) -> (key: String, value: String)? {
        ...
        private static func _get(fromEnviron environ: UnsafeMutablePointer<UnsafeMutablePointer<CChar>?>)
            -> [String: String]
        {
        ...
        private static let _environ_lock_np = symbol(named: "environ_lock_np").map {
        ...
        /// system, the value of this property is `nil`.
        private static let _environ_unlock_np = symbol(named: "environ_unlock_np").map {
            unsafeBitCast($0, to: (@convention(c) () -> Void).self)
        }
    #endif
            ...
            return name.withCString { name in
                for i in 0... {
                    ...
                    if let equals = strchr(rowp, CInt(UInt8(ascii: "="))) {
                        ...
                        if strncmp(rowp, name, keyLength) == 0 {
                    ...
                    withUnsafeTemporaryAllocation(of: wchar_t.self, capacity: maxCount) { buffer in
                        ...
                        if count == 0 {
                            ...
                            case let errorCode:
                                ...
                                fatalError(
                                    "Unexpected error when getting environment variable '\(name)': \(error) (\(errorCode))"
                                )
                            }
