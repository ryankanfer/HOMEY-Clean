    ...
    init?(identifying value: some Sendable, parameter: Test.Parameter) throws {
            ...
            let encodableValue: (any Encodable)? = if let customEncodable = value as? any CustomTestArgumentEncodable {
                customArgumentWrapper(for: customEncodable)
                      ...
                      let encodableRawValue = rawRepresentable.rawValue as? any Encodable
            {
                encodableRawValue
            } else if let encodable = value as? any Encodable {
                encodable
            } else if let identifiable = value as? any Identifiable,
                      let encodableID = identifiable.id as? any Encodable
            {
                encodableID
            } else {
                nil
            ...
            self = try .init(bytes: Self._encode(encodableValue, parameter: parameter))
        #else
            nil
        #endif
    }
...
private extension CodingUserInfoKey {
    ...
    static var _testParameterUserInfoKey: Self {
