        ...
        withUnsafeTemporaryAllocation(of: UnsafeMutableRawPointer?.self, capacity: addressCount) { addresses in
            var initializedCount = 0
                ...
                if #available(_backtraceAsyncAPI, *) {
                    ...
                    initializedCount = .init(clamping: backtrace(
                        addresses.baseAddress!,
                        .init(clamping: addresses.count)
                    ))
                }
            #elseif os(Android)
            ...
            #elseif os(Windows)
                initializedCount = Int(clamping: RtlCaptureStackBackTrace(
                    0,
                    ULONG(clamping: addresses.count),
                    addresses.baseAddress!,
                    nil
                ))
            #elseif os(WASI)
...
extension Backtrace {
    ...
    private static func _willThrow(
        _ errorObject: AnyObject,
    ...
    private static func _willThrowTyped<E>(_: borrowing E, from backtrace: Backtrace) where E: Error {
    ...
    ///   - backtrace: The backtrace from where the error was thrown.
    @available(_typedThrowsAPI, *)
    private static func _willThrowTyped(
        _ errorAddress: UnsafeMutableRawPointer,
        _ errorType: UnsafeRawPointer,
        _ errorConformance: UnsafeRawPointer,
        from backtrace: Backtrace
    ) {
        ...
        func forward<E>(_: E.Type) where E: Error {
        ...
        if Environment.flag(named: "SWT_SWIFT_ERROR_BACKTRACING_ENABLED") != false {
            ...
            if #available(_typedThrowsAPI, *) {
                _oldWillThrowTypedHandler.withLock { oldWillThrowTypedHandler in
                    oldWillThrowTypedHandler =
                        swt_setWillThrowTypedHandler { errorAddress, errorType, errorConformance in
                            let backtrace = Backtrace.current()
