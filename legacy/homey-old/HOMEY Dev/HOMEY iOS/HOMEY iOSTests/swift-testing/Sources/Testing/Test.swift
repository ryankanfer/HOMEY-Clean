...
public struct Test: Sendable {
    ...
    var testCases: (some Sequence<Test.Case>)? {
        testCasesState.map { testCasesState in
            guard case let .evaluated(testCases) = testCasesState else {
                ...
                preconditionFailure(
                    "Attempting to access test cases with invalid state. Please file a bug report at https://github.com/swiftlang/swift-testing/issues/new and include this information: \(String(reflecting: testCasesState))"
                )
            }
            return testCases
        }
    ...
    mutating func evaluateTestCases() async throws {
        if case let .unevaluated(function) = testCasesState {
            do {
                ...
                testCasesState = .evaluated(sequence)
            } catch {
                testCasesState = .failed(error)
                throw error
            }
        }
    }
    ...
    init<S>(
        ...
        testCasesState = .unevaluated { try .init(await testCases()) }
    ...
    init<S>(
        ...
        self.containingTypeInfo = containingTypeInfo
        self.xcTestCompatibleSelector = xcTestCompatibleSelector
        testCasesState = .evaluated(.init(testCases))
        self.parameters = parameters
    }
}
...
extension Test: Equatable, Hashable {
    ...
    public extension Test {
        ...
        @_spi(ForToolsIntegrationOnly)
        struct Snapshot: Sendable, Codable, Identifiable {
