...
public extension Backtrace {
    ...
    enum SymbolicationMode: Sendable {
    ...
    struct SymbolicatedAddress: Sendable {
    ...
    func symbolicate(_ mode: SymbolicationMode) -> [SymbolicatedAddress] {
        var result = addresses.map { SymbolicatedAddress(address: $0) }
                ...
                for (i, address) in addresses.enumerated() {
                    var info = Dl_info()
                    if dladdr(UnsafePointer(bitPattern: UInt(clamping: address)), &info) != 0 {
                ...
                _withDbgHelpLibrary { hProcess in
                    ...
                    for (i, address) in addresses.enumerated() {
                        withUnsafeTemporaryAllocation(of: SYMBOL_INFO_PACKAGEW.self, capacity: 1) { symbolInfo in
                            ...
                            if SymFromAddrW(
                                hProcess,
                                DWORD64(clamping: address),
                                &displacement,
                                symbolInfo.pointer(to: \.si)!
                            ) {
                                let symbolName = String.decodeCString(
                                    symbolInfo.pointer(to: \.si.Name)!,
                                    as: UTF16.self
                                )?.result
                                result[i] = SymbolicatedAddress(
                                    address: address,
                                    offset: displacement,
