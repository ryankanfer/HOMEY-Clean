...
public extension Event {
    ...
    struct HumanReadableOutputRecorder: Sendable /* , ~Copyable */ {
...
extension Event.HumanReadableOutputRecorder {
    ...
    private func _issueCounts(in graph: Graph<String, Event.HumanReadableOutputRecorder._Context.TestData?>?)
        -> (issueCount: Int, knownIssueCount: Int, totalIssueCount: Int, description: String)
    {
...
private extension Test.Case {
    ...
    func labeledArguments(includingQualifiedTypeNames includeTypeNames: Bool = false) -> String {
        arguments.lazy
            .map { argument in
...
public extension Event.HumanReadableOutputRecorder {
    ...
    @discardableResult func record(
        ...
        case let .testSkipped(skipInfo):
            ...
            return if let comment = skipInfo.comment {
                [
                    Message(
                        symbol: .skip,
                        stringValue: "\(_capitalizedTitle(for: test)) \(testName) skipped: \"\(comment.rawValue)\""
                    ),
                ]
            } else {
                [
        ...
        case let .issueRecorded(issue):
            ...
            if case let .expectationFailed(expectation) = issue.kind,
               let differenceDescription = expectation.differenceDescription
            {
            ...
            let primaryMessage = if parameterCount == 0 {
                Message(
                    symbol: symbol,
