...
public extension Result {
    ...
    @inlinable func __expected() where Success == Void {}

    /// Handle this instance as if it were returned from a call to `#require()`.
    ///
    /// - Warning: This function is used to implement the `#expect()` and
    ///   `#require()` macros. Do not call it directly.
    @inlinable func __required() throws -> Success {
        try get()
    }
}
...
public extension Result {
    ...
    /// - Warning: This function is used to implement the `#expect()` and
    ///   `#require()` macros. Do not call it directly.
    @discardableResult @inlinable func __expected<T>() -> Success where Success == T? {
        try? get()
    }
    ...
    /// - Warning: This function is used to implement the `#expect()` and
    ///   `#require()` macros. Do not call it directly.
    @discardableResult func __required<T>() throws -> T where Success == T? {
        guard let result = try get() else {
            throw APIMisuseError(
                description: "Could not unwrap 'nil' value of type Optional<\(T.self)>. Consider using #expect() instead of #require() here."
            )
        }
        return result
    }
