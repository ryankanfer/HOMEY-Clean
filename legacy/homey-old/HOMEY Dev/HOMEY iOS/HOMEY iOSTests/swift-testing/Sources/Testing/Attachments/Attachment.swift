...
// MARK: - Initializing an attachment
    ...
    public extension Attachment where AttachableValue: ~Copyable {
    ...
    fileprivate extension Attachment where AttachableValue == AnyAttachable {
...
public struct AnyAttachable: AttachableContainer, Copyable, Sendable {
    ...
    public func withUnsafeBufferPointer<R>(
        ...
        func open<T>(_ attachableValue: T, for attachment: borrowing Attachment<Self>) throws -> R
            where T: Attachable & Sendable & Copyable
        {
...
public extension Attachment where AttachableValue: ~Copyable {
    ...
    @_disfavoredOverload var attachableValue: AttachableValue {
        _read {
...
@_spi(Experimental)
public extension Attachment where AttachableValue: AttachableContainer & ~Copyable {
    /// The value of this attachment.
    ///
    ...
    /// ```
    var attachableValue: AttachableValue.AttachableValue {
        _read {
...
#if !SWT_NO_LAZY_ATTACHMENTS
    public extension Attachment where AttachableValue: Sendable & Copyable {
        ...
        consuming func attach(sourceLocation: SourceLocation = #_sourceLocation) {
...
public extension Attachment where AttachableValue: ~Copyable {
    ...
    @inlinable borrowing func withUnsafeBufferPointer<R>(_ body: (UnsafeRawBufferPointer) throws -> R) throws -> R {
    ...
    extension Attachment where AttachableValue: ~Copyable {
        ...
        borrowing func write(
            toFileInDirectoryAtPath directoryPath: String,
            usingPreferredName: Bool = true,
            appending suffix: @autoclosure () -> String
        ) throws -> String {
            let result: String
    ...
    extension Configuration {
        ...
        func handleValueAttachedEvent(_ event: inout Event, in _: borrowing Event.Context) -> Bool {
            ...
            guard case let .valueAttached(attachment) = event.kind else {
                preconditionFailure(
                    "Passed the wrong kind of event to \(#function) (expected valueAttached, got \(event.kind)). Please file a bug report at https://github.com/swiftlang/swift-testing/issues/new"
                )
            }
            ...
            do {
                ...
                Issue(kind: .valueAttachmentFailed(error), comments: [], sourceContext: sourceContext)
                    .record(configuration: self)
                return false
            }
        }
    }
