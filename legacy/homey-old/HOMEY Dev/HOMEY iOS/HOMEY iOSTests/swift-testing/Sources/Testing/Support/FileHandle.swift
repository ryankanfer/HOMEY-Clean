...
internal import _TestingInternals
        ...
        extension FileHandle {
            ...
            static func makePipe(readEnd: inout FileHandle?, writeEnd: inout FileHandle?) throws {
                var (fdReadEnd, fdWriteEnd) = try withUnsafeTemporaryAllocation(of: CInt.self, capacity: 2) { fds in
                    #if os(Windows)
                        guard _pipe(fds.baseAddress, 0, _O_BINARY) == 0 else {
                            throw CError(rawValue: swt_errno())
                        }
                    #else
                        guard pipe(fds.baseAddress!) == 0 else {
                            throw CError(rawValue: swt_errno())
                        }
                    #endif
                    return (fds[0], fds[1])
    ...
    extension FileHandle {
        ...
        var isTTY: Bool {
                ...
                withUnsafePOSIXFileDescriptor { fd in
                    if let fd, isatty(fd) != 0, let term = Environment.variable(named: "TERM"), !term.isEmpty {
                        return true
                    }
                    return false
                }
            ...
            var isPipe: Bool {
                    ...
                    withUnsafePOSIXFileDescriptor { fd in
                        ...
                        var statStruct = stat()
                        return fstat(fd, &statStruct) == 0 && swt_S_ISFIFO(mode_t(statStruct.st_mode))
                    }
                #elseif os(Windows)
                    return withUnsafeWindowsHANDLE { handle in
                        guard let handle else {
            ...
            path.withCString(encodedAs: UTF16.self) { path in
                pathComponent.withCString(encodedAs: UTF16.self) { pathComponent in
                    withUnsafeTemporaryAllocation(
                        of: wchar_t.self,
                        capacity: (wcslen(path) + wcslen(pathComponent)) * 2 + 1
                    ) { buffer in
