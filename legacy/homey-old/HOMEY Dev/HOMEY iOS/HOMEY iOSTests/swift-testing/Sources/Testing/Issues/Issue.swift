...
extension Issue.Kind: CustomStringConvertible {
        ...
        struct Snapshot: Sendable, Codable {
            ...
            public init(snapshotting issue: borrowing Issue) {
                if case .confirmationMiscounted = issue.kind {
                    ...
                    kind = .unconditional
                    comments = CollectionOfOne("\(issue.kind)") + issue.comments
                } else {
                    kind = Issue.Kind.Snapshot(snapshotting: issue.kind)
    ...
    public extension Issue.Kind {
        /// Serializable kinds of issues which may be recorded.
        @_spi(ForToolsIntegrationOnly)
        enum Snapshot: Sendable, Codable {
            /// An issue which occurred unconditionally, for example by using
            /// ``Issue/record(_:sourceLocation:)``.
            case unconditional
            ...
            public init(from decoder: any Decoder) throws {
                ...
                if try container.decodeIfPresent(Bool.self, forKey: .unconditional) != nil {
                    self = .unconditional
                } else if let expectationFailedContainer = try? container.nestedContainer(
                    keyedBy: _CodingKeys._ExpectationFailedKeys.self,
                    forKey: .expectationFailed
                ) {
                    self = try .expectationFailed(expectationFailedContainer.decode(
                        Expectation.Snapshot.self,
                        forKey: .expectation
                    ))
                } else if let confirmationMiscountedContainer = try? container.nestedContainer(
                    keyedBy: _CodingKeys._ConfirmationMiscountedKeys.self,
                    forKey: .confirmationMiscounted
                ) {
                    self = try .confirmationMiscounted(
                        actual: confirmationMiscountedContainer.decode(
                            Int.self,
                            forKey: .actual
                        ),
                        expected: confirmationMiscountedContainer.decode(
                            Int.self,
                            forKey: .expected
                        )
                    )
                } else if let errorCaught = try? container.nestedContainer(
                    keyedBy: _CodingKeys._ErrorCaughtKeys.self,
                    forKey: .errorCaught
                ) {
                    self = try .errorCaught(errorCaught.decode(ErrorSnapshot.self, forKey: .error))
            ...
            public func encode(to encoder: any Encoder) throws {
                ...
                case let .expectationFailed(expectation):
                    var errorCaughtContainer = container.nestedContainer(
                        keyedBy: _CodingKeys._ExpectationFailedKeys.self,
                        forKey: .expectationFailed
                    )
                    try errorCaughtContainer.encode(expectation, forKey: .expectation)
                case let .confirmationMiscounted(actual, expected):
                    var confirmationMiscountedContainer = container.nestedContainer(
                        keyedBy: _CodingKeys._ConfirmationMiscountedKeys.self,
                        forKey: .confirmationMiscounted
                    )
