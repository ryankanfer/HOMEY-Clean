...
internal import _TestingInternals
    ...
    func spawnExecutable(
        ...
        // use, so use this typealias to paper over the differences.
            ...
            return try withUnsafeTemporaryAllocation(
                of: P<posix_spawn_file_actions_t>.self,
                capacity: 1
            ) { fileActions in
                ...
                return try withUnsafeTemporaryAllocation(of: P<posix_spawnattr_t>.self, capacity: 1) { attrs in
                    ...
                    func inherit(_ fileHandle: borrowing FileHandle, as standardFD: CInt? = nil) throws {
                        try fileHandle.withUnsafePOSIXFileDescriptor { fd in
                            guard let fd else {
                                throw SystemError(
                                    description: "A child process cannot inherit a file handle without an associated file descriptor. Please file a bug report at https://github.com/swiftlang/swift-testing/issues/new"
                                )
                            }
                            if let standardFD {
                    ...
                    var pid = pid_t()
                    guard posix_spawn(&pid, executablePath, fileActions, attrs, argv, environ) == 0 else {
                        throw CError(rawValue: swt_errno())
                    }
                    return pid
                }
                ...
                func inherit(_ fileHandle: borrowing FileHandle, as outWindowsHANDLE: inout HANDLE?) throws {
                    try fileHandle.withUnsafeWindowsHANDLE { windowsHANDLE in
                        guard let windowsHANDLE else {
                            throw SystemError(
                                description: "A child process cannot inherit a file handle without an associated Windows handle. Please file a bug report at https://github.com/swiftlang/swift-testing/issues/new"
                            )
                        }
    ...
    // MARK: -
        ...
        private func _withStartupInfoEx<R>(
            attributeCount: Int = 0,
            _ body: (UnsafeMutablePointer<STARTUPINFOEXW>) throws -> R
        ) throws -> R {
