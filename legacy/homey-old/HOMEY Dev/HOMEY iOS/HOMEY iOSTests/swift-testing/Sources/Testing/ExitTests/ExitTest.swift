...
public struct ExitTest: Sendable, ~Copyable {
    ...
    func callExitTest(
        ...
        do {
            ...
            result = try await configuration.exitTestHandler(exitTest)
                ...
                if case let .exitCode(exitCode) = result.exitCondition,
                   (exitCode & ~STATUS_CODE_MASK) == STATUS_SIGNAL_CAUGHT_BITS
                {
    ...
    extension ExitTest {
        ...
        private static let _backChannelForEntryPoint: FileHandle? = {
            guard let backChannelEnvironmentVariable = Environment.variable(named: "SWT_EXPERIMENTAL_BACKCHANNEL")
            else {
                return nil
            }
        ...
        static func handlerForEntryPoint() -> Handler {
            ...
            let childArguments: [String] = {
                ...
                let parentArguments = CommandLine.arguments
                    ...
                    if let executablePath = try? childProcessExecutablePath.get() {
                        executablePath.withCString { childProcessExecutablePath in
                            withUnsafeTemporaryAllocation(
                                of: CChar.self,
                                capacity: strlen(childProcessExecutablePath) + 1
                            ) { baseName in
                                if basename_r(childProcessExecutablePath, baseName.baseAddress!) != nil {
                                    isHostedByXCTest = strcmp(baseName.baseAddress!, "xctest") == 0
                                }
                            }
                        }
                    }
                    ...
                    let hasTestingLibraryArgument: Bool = parentArguments
                        .contains { $0.starts(with: "--testing-library") }
            ...
            return { exitTest in
                ...
                try JSON.withEncoding(of: exitTest.sourceLocation) { json in
                    childEnvironment["SWT_EXPERIMENTAL_EXIT_TEST_SOURCE_LOCATION"] = String(
                        decoding: json,
                        as: UTF8.self
                    )
                }
        ...
        private static func _processRecord(
            _ recordJSON: UnsafeRawBufferPointer,
            fromBackChannel _: borrowing FileHandle
        ) throws {
