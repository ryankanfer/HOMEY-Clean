...
func parseCommandLineArguments(from args: [String]) throws -> __CommandLineArguments_v0 {
    ...
    func isLastArgument(at index: [String].Index) -> Bool {
        args.index(after: index) >= args.endIndex
            ...
            if let configurationIndex = args.firstIndex(of: "--configuration-path") ?? args
                .firstIndex(of: "--experimental-configuration-path"),
            ...
            if let eventOutputIndex = args.firstIndex(of: "--event-stream-output-path") ?? args
                .firstIndex(of: "--experimental-event-stream-output"),
                !isLastArgument(at: eventOutputIndex)
            {
                result.eventStreamOutputPath = args[args.index(after: eventOutputIndex)]
            }
            // Event stream output (experimental)
            if let eventOutputVersionIndex = args.firstIndex(of: "--event-stream-version") ?? args
                .firstIndex(of: "--experimental-event-stream-version"),
        ...
        // Attachment output
        if let attachmentsPathIndex = args.firstIndex(of: "--experimental-attachments-path"),
    ...
    if let symbolicateBacktracesIndex = args.firstIndex(of: "--symbolicate-backtraces"),
       !isLastArgument(at: symbolicateBacktracesIndex)
    {
...
public func configurationForEntryPoint(from args: __CommandLineArguments_v0) throws -> Configuration {
    ...
    if let symbolicateBacktraces = args.symbolicateBacktraces {
        ...
        if let attachmentsPath = args.experimentalAttachmentsPath {
            ...
            if let eventStreamOutputPath = args.eventStreamOutputPath {
                ...
                let eventHandler = try eventHandlerForStreamingEvents(
                    version: args.eventStreamVersion,
                    encodeAsJSONLines: true
                ) { json in
                    _ = try? file.withLock {
                        try file.write(json)
                        try file.write("\n")
    ...
    func testFilter(
        forRegularExpressions regexes: [String]?,
        label: String,
        membership: Configuration.TestFilter.Membership
    ) throws -> Configuration.TestFilter {
    ...
    try filters.append(testFilter(forRegularExpressions: args.filter, label: "--filter", membership: .including))
    try filters.append(testFilter(forRegularExpressions: args.skip, label: "--skip", membership: .excluding))
        ...
        static func `for`(_ fileHandle: borrowing FileHandle) -> Self {
            ...
            if result.useANSIEscapeCodes {
                ...
                if let environmentVariable = Environment.flag(named: "SWT_SF_SYMBOLS_ENABLED") {
                    ...
                    result.useSFSymbols = (access("/Library/Fonts/SF-Pro.ttf", F_OK) == 0)
                }
            #endif
