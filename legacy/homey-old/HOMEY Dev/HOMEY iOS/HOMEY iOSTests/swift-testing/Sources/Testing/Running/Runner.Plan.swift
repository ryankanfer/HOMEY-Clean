...
extension Runner.Plan {
    ...
    /// node.
    private static func _recursivelyApplyTraits(
        _ parentTraits: [any SuiteTrait] = [],
        to testGraph: inout Graph<String, Test?>
    ) {
    ...
    private static func _recursivelySynthesizeSuites(
        in graph: inout Graph<String, Test?>,
        ...
        func synthesizeSuites(
            in graph: inout Graph<String, Test?>,
            nameComponents: [String] = [],
            sourceLocation: inout SourceLocation?
        ) {
            for (key, var childGraph) in graph.children {
                synthesizeSuites(
                    in: &childGraph,
                    nameComponents: nameComponents + [key],
    ...
    private static func _constructStepGraph(
        from tests: some Sequence<Test>,
        configuration: Configuration
    ) async -> Graph<String, Step?> {
        ...
        testGraph = await testGraph.mapValues { keyPath, test in
            ...
            if case .run = action, let testCases = test.testCases, testCases.first(where: { _ in true }) == nil {
                action = .skip(SkipInfo(
                    ...
                    sourceContext: .init(backtrace: nil, sourceLocation: test.sourceLocation)
                ))
            }
    ...
    public extension Runner.Plan {
        ...
        struct Snapshot: Sendable {
            ...
            public init(snapshotting plan: borrowing Runner.Plan) {
                for (keyPath, step) in plan.stepGraph {
    ...
    public extension Runner.Plan.Step {
    ...
    public extension Runner.Plan.Action {
