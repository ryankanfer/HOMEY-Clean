        ...
        get throws {
                ...
                while result == nil {
                    withUnsafeTemporaryAllocation(of: CChar.self, capacity: Int(bufferCount)) { buffer in
                        ...
                        if _NSGetExecutablePath(buffer.baseAddress, &bufferCount) == 0 {
                ...
                return try mib.withUnsafeMutableBufferPointer { mib in
                    var bufferCount = 0
                    guard sysctl(mib.baseAddress!, .init(mib.count), nil, &bufferCount, nil, 0) == 0 else {
                        throw CError(rawValue: swt_errno())
                    }
                    return try withUnsafeTemporaryAllocation(of: CChar.self, capacity: bufferCount) { buffer in
                        guard
                            == 0 sysctl(mib.baseAddress!, .init(mib.count), buffer.baseAddress!, &bufferCount, nil, 0)
                        else {
                            throw CError(rawValue: swt_errno())
                        }
                        return String(cString: buffer.baseAddress!)
