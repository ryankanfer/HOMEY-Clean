...
public extension Event {
    ...
    struct JUnitXMLRecorder: Sendable /* , ~Copyable */ {
...
extension Event.JUnitXMLRecorder {
    ...
    private func _record(_ event: borrowing Event, in eventContext: borrowing Event.Context) -> String? {
        ...
        case .runStarted:
            ...
            """#
        case .testStarted where test?.isSuite == false:
            let id = test!.id
            ...
            return nil
        case .testEnded where test?.isSuite == false:
            let id = test!.id
            let keyPath = id.keyPathRepresentation
            _context.withLock { context in
            ...
            return nil
        case let .testSkipped(skipInfo) where test?.isSuite == false:
            let id = test!.id
            let keyPath = id.keyPathRepresentation
            _context.withLock { context in
        ...
        case .runEnded:
            return _context.withLock { context in
                ...
                return #"""
                  <testsuite name="TestResults" errors="0" tests="\#(context
                    .testCount
                )" failures="\#(issueCount)" skipped="\#(skipCount)" time="\#(durationSeconds)">
