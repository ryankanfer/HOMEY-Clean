...
public extension Test {
    ...
    ///
    /// This clock tracks time using both the [suspending
    /// clock](https://developer.apple.com/documentation/swift/suspendingclock)
...
public extension SuspendingClock.Instant {
    ...
    init(_ testClockInstant: Test.Clock.Instant) {
        self.init(testClockInstant.suspending)
    }
}

public extension Test.Clock.Instant {
    #if !SWT_NO_UTC_CLOCK
        ...
        /// calculations.
        var timeComponentsSince1970: (seconds: Int64, attoseconds: Int64) {
            wall.components
        }
        ...
        /// calculations.
        @available(_clockAPI, *)
        var durationSince1970: Duration {
            Duration(wall)
        }
    #endif
    ...
    internal func nanoseconds(until other: Self) -> Int64 {
        if other < self {
            return -other.nanoseconds(until: self)
        }
        let otherNanoseconds = (other.suspending.seconds * 1_000_000_000) +
            (other.suspending.attoseconds / 1_000_000_000)
...
extension Test.Clock {
    ...
    static func sleep(for duration: Duration) async throws {
            ...
            var tsRemaining = ts
            while nanosleep(&ts, &tsRemaining) != 0 {
...
extension Test.Clock.Instant {
    ...
    func descriptionOfDuration(to other: Test.Clock.Instant) -> String {
        #if SWT_TARGET_OS_APPLE
            let (seconds, nanosecondsRemaining) = nanoseconds(until: other)
