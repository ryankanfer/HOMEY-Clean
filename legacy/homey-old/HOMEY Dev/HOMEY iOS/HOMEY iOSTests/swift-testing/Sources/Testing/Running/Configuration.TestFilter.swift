...
public extension Configuration.TestFilter {
    ...
    init(including testIDs: some Collection<Test.ID>) {
    ...
    init(excluding testIDs: some Collection<Test.ID>) {
    ...
    @available(_regexAPI, *)
    internal init(membership: Membership, matchingAnyOf patterns: some Sequence<String>) throws {
    ...
    init(includingAnyOf tags: some Collection<Tag>) {
    ...
    ///
    /// Matching tests have had _any_ of the tags in `tags` added to them.
    init(excludingAnyOf tags: some Collection<Tag>) {
    ...
    init(includingAllOf tags: some Collection<Tag>) {
    ...
    ///
    /// Matching tests have had _all_ of the tags in `tags` added to them.
    init(excludingAllOf tags: some Collection<Tag>) {
...
private extension Configuration.TestFilter {
    /// An enumeration which represents filtering logic to be applied to a test
    /// graph.
    enum Operation<Item>: Sendable where Item: _FilterableItem {
...
extension Configuration.TestFilter.Kind {
    ...
    func operation<T>(itemType _: T.Type = T.self) throws -> Configuration.TestFilter.Operation<T>
        where T: _FilterableItem
    {
        switch self {
        case .unfiltered:
            return .unfiltered
...
private extension Configuration.TestFilter.Operation {
    ...
    func apply(to testGraph: Graph<String, Item?>) -> Graph<String, Item?> {
        switch self {
        case .unfiltered:
            return testGraph
...
public extension Configuration.TestFilter {
    ...
    enum CombinationOperator: Sendable {
    ...
    func combining(with other: Self, using op: CombinationOperator = .and) -> Self {
    ...
    mutating func combine(with other: Self, using op: CombinationOperator = .and) {
...
private extension Configuration.TestFilter.Kind {
    ...
    var requiresTraitPropagation: Bool {
        switch self {
        case .unfiltered,
             .testIDs,
             .patterns:
...
/// is `true`.
private struct FilterItem: _FilterableItem {
    var test: Test
