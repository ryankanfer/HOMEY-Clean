        ...
        init(for attachment: borrowing Attachment<some Attachable>) throws {
                ...
                if #available(_uttypesAPI, *), let contentType = UTType(filenameExtension: ext) {
                    if contentType == .data {
                        ...
                        throw CocoaError(
                            ...
                            userInfo: [
                                NSLocalizedDescriptionKey: "The content type '\(contentTypeDescription)' cannot be used to attach an instance of \(type(of: self)) to a test.",
                            ]
                        )
                    }
                    return
            ...
            if ext.isEmpty {
                ...
                throw CocoaError(
                    .propertyListWriteInvalid,
                    userInfo: [
                        NSLocalizedDescriptionKey: "The path extension '.\(ext)' cannot be used to attach an instance of \(type(of: self)) to a test.",
                    ]
                )
            }
        }
